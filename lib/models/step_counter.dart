...
class StepCounter {
  ...
  Future<void> startListening() async {
    if (_streamSubscription == null) {
      _streamSubscription = Stream.periodic(Duration(milliseconds: 300)).asyncMap((_) => accelerometerEvents.first).listen(_onAccelerometerData);
    }
  }
  ...
  bool _isPeak(double X, double Y, double Z) {
    ...
    bool isPeak = (X - Z > 1 && Y - Z > 1 && absXYZ - prevAbsXYZ > 0.1);
    // if (isPeak) {
    //   // Check if the peak is within the threshold
    //   double value = prevAbsXYZ - absXYZ;
    //   isPeak = value.abs() > _xyzThreshold;
    // }
    if(isPeak){
